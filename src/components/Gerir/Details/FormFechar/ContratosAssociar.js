...
const ContratosAssociar = ({initialData, handler, setFiltered, filtered}) => {
    ...
    const changeHandler = () => {
        ...
        const filteredData = initialData.filter(contrato => {
            return contrato.Entity.includes(valor) || contrato.NIF.toString().includes(valor) || 
            contrato.Id.includes(valor) || contrato.BusinessLine.includes(valor)
        })
